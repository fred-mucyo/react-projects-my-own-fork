...
export async function loader(){
    ...
    getPosts(), getUsers(), getCategories()
    ])
...
const displayedPosts = categoryFilter ? posts.filter(post => post.categoryId === categoryFilter) : posts
    ...
    return <div className="bg-stone-50 min-h-screen px-6 py-10 md:px-12 lg:px-20 ">
...
<div>{<img src={post.coverImage}  alt={post.title} className="w-100"/>}</div>
...
<div className="w-5 ml-3.5 rounded-2xl overflow-hidden">{<img src={avatar?.avatar}/>}</div>
<div>{author?.name}</div>
